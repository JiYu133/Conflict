...
`DeathType` 支持 `FRONT`、`BACK`、`RIGHT`、`FRONT_HEADSHOT`、`BACK_HEADSHOT`、`CROUCHING_HEADSHOT`、`EXPLOSION` 和 `GENERIC`。类型用于选择死亡动画、默认冲击方向和动能传递比例；调用方也可以传入明确的世界空间冲击方向。
...
### `enable(death_type = GENERIC, impact_direction = Vector3.ZERO, impact_energy_j = 0.0, impact_mass_kg = 0.0, impact_damage_type = BULLET) -> void`
...
5. 进入物理阶段时隐藏武器、调整第一人称网格渲染层、启动模拟；如果调用方提供命中能量和质量，则按 `p = sqrt(2*m*E)` 计算冲量并施加。
...
- 冲量优先施加到上半身关键词匹配的骨骼，并按各骨骼质量比例分配；爆炸和爆头使用各自的能量传递比例。

## 模型内预制物理骨骼

测试模型 `assets/models/player/test_model/swat.tscn` 包含一个编辑器生成的 `PhysicalBoneSimulator3D` 和 21 个主要身体物理骨骼，便于在编辑器中检查骨骼映射、关节和碰撞形状。它**不是**运行时布娃娃的数据源。
